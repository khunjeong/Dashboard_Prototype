<template>
  <div class="co-chart-wrapper" ref="wrapper"></div>
</template>
...
@Component({})
export default class CoChart extends Vue {
  async mounted() {
    try {
      ...
      const co2Data = await d3.csv("/assets/co.csv");
      ...
      const co2Path = svg
        ...
        .on("mouseover", () => {
          co2Path.transition().attr("stroke-width", 9);
          yLeftG.attr("class", "co");
        })
        .on("mouseout", () => {
          co2Path.transition().attr("stroke-width", 5);
          yLeftG.attr("class", "");
        });
    } catch (e) {
      console.log("error", e);
